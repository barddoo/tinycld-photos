...
describe("photoToView", () => {
	...
	it("preserves null for latitude and longitude", () => {
		const view = photoToView(
			photoItem("p1", { latitude: null, longitude: null }),
		);
	...
	it("maps numeric latitude/longitude when present", () => {
		const view = photoToView(
			photoItem("p1", { latitude: 48.8566, longitude: 2.3522 }),
		);
	...
	it("maps numeric dimensions and size", () => {
		const view = photoToView(
			photoItem("p1", { width: 3840, height: 2160, size: 10485760 }),
		);
