...
export function enqueue(files: File[]): string[] {
    ...
    const entries: UploadEntry[] = files.map(f => {
        const id = nextId()
        ids.push(id)
        return { id, fileName: f.name, fileSize: f.size, status: 'pending', file: f }
    })
...
export function updateStatus(id: string, status: UploadStatus, error?: string) {
    state = {
        ...state,
        entries: state.entries.map(e => (e.id === id ? { ...e, status, error } : e)),
    }
    notify()
}
...
export function clearDone() {
    state = { ...state, entries: state.entries.filter(e => e.status !== 'done') }
    notify()
}
