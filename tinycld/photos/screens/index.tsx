...
export default function PhotosTimeline({ section: _section }: Props) {
	...
	const { uploadPhotos, restorePhoto, permanentlyDelete, retryUpload } =
	...
	const renderItem = useCallback(
		({ item }: { item: ListRow }) => {
			...
			if (item.kind === "photo") {
				return (
					<View style={PHOTO_CELL_STYLE}>
						<PhotoCard
							photo={item.photo}
							size={cardSize}
							onPress={handlePhotoPress}
							onLongPress={
								section === "trash" ? handlePhotoLongPress : undefined
							}
						/>
					</View>
				);
