...
export default function MLSettings() {
		...
		try {
			...
			if (s.settings) {
				setConfig((prev) => ({
					...
					maxFaceDistance: String(
						s.settings.max_face_distance ?? prev.maxFaceDistance,
					),
	...
	const updateField = useCallback(
		<K extends keyof MLConfig>(key: K, value: MLConfig[K]) => {
	...
	return (
		<ScrollView style={{ flex: 1, backgroundColor: bg }}>
			...
			<Text className="px-4 pb-3 text-xs" style={{ color: muted }}>
				Controls how often the ML pipeline checks for new work and how many
				photos it processes at once.
			</Text>
			<View
				className="mx-4 rounded-xl overflow-hidden"
				style={{ backgroundColor: surface }}
			...
			<Text className="px-4 pb-3 text-xs" style={{ color: muted }}>
				Extract visible text from photos. When enabled, text in images becomes
