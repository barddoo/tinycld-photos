...
export function useTags(orgId: string, userOrgId: string) {
    ...
    const { data: tags, isLoading } = useOrgLiveQuery(
        (query, { orgId }) =>
            query
    ...
    const updateTag = useCallback(async (tagId: string, data: { name?: string; color?: string }) => {
...
export function usePhotoTags(photoId: string) {
    ...
    const { data: itemTags } = useOrgLiveQuery(
        (query) =>
            query
                .from({ it: itemTagsCollection })
                .where(({ it }) => eq(it.item, photoId)),
        [photoId],
    )
    ...
    const removeTag = useCallback(async (itemTagId: string) => {
